...
1. INSTALL REQUIRED PACKAGES:
   pip install python-whois openai python-dotenv
...
def load_checked_domains() -> Set[str]:
    ...
    if os.path.exists(CHECKED_DOMAINS_FILE):
        with open(CHECKED_DOMAINS_FILE, 'r') as f:
            for line in f:
                domain = line.strip().lower()  # Convert to lowercase for case-insensitive comparison
                if domain:
...
def save_checked_domains(domains: List[str]) -> None:
    ...
    new_domains = [domain for domain in domains if domain.lower() not in checked_domains]
...
def filter_already_checked_domains(company_names: List[str], checked_domains: Set[str]) -> List[str]:
    """
    ...
    if filtered_out_count > 0:
        print(f"Filtered out {filtered_out_count} company names that would result in already checked domains")
    
    return filtered_names

def generate_company_names(prompt: str, num_names: int = 50, model: str = "gpt-4o", checked_domains: Set[str] = None) -> List[str]:
    """
    ...
    try:
        ...
        if checked_domains:
            checked_domains_sample = list(checked_domains)[-20:] if len(checked_domains) > 20 else list(checked_domains)
        ...
        try:
            ...
            print(f"Error with JSON response format: {str(e)}. Trying without explicit format...")
            
            # Fallback to standard completion without JSON format specification
            response = client.chat.completions.create(
                model=model,
                messages=[
                    {"role": "system", "content": system_message + " Format your response as a valid JSON array."},
            ...
            try:
                ...
                for line in lines:
                    ...
                    name_match = re.search(r'(?:^|\s)(?:\d+\.\s*)?([A-Z][a-zA-Z0-9]*(?:\s[A-Z][a-zA-Z0-9]*)?)', line)
                    if name_match and len(name_match.group(1)) > 2:  # Ensure name is at least 3 chars
    ...
    for idx, name in enumerate(company_names, 1):
        ...
        for attempt in range(max_retries):
            try:
                ...
                if isinstance(domain.status, (type(None), str)) and "No match for" in str(domain):
                ...
                if "No match for" in str(e):
                    ...
                    if attempt < max_retries - 1:  # if we still have retries left
                        print(f"  Connection error, retry {attempt+1}/{max_retries} in {wait_time:.1f} seconds...")
                        time.sleep(wait_time)
                        continue
                    else:
                        available_domains[domain_name] = f"Connection error after {max_retries} attempts"
                else:
        ...
        if idx < total:  # Don't delay after the last request
            ...
            if consecutive_failures > 2:
                ...
                print(f"  Increasing delay due to consecutive failures: {current_delay_range[0]:.1f}-{current_delay_range[1]:.1f}s")
            elif consecutive_failures == 0 and current_delay_range[0] > initial_delay_range[0]:
...
def save_available_domains(domains: List[str], existing_domains: Set[str]) -> None:
    ...
    # Filter out domains that already exist in the file (case insensitive)
    new_domains = [domain for domain in domains if domain.lower() not in existing_domains]
...
def auto_generate_and_check(
    ...
    print(f"Loaded {len(existing_domains)} existing domains from {AVAILABLE_DOMAINS_FILE}")
    ...
    if len(company_names) == 0:
        print("All generated domains have already been checked. Please try again with a different prompt.")
        return
    
    if len(company_names) < original_count:
        print(f"Filtered out {original_count - len(company_names)} already checked domains. Proceeding with {len(company_names)} unique names.")
...
def main():
    parser = argparse.ArgumentParser(description='Generate company names and check domain availability')
    parser.add_argument('--prompt', '-p', type=str, help='Description of the type of company names to generate')
    parser.add_argument('--num', '-n', type=int, default=50, help='Number of names to generate (default: 50)')
    parser.add_argument('--model', '-m', type=str, default='gpt-4o', help='OpenAI model to use (default: gpt-4o)')
    parser.add_argument('--min-delay', type=float, default=0.2, help='Minimum initial delay between requests in seconds (default: 0.2)')
    parser.add_argument('--max-delay', type=float, default=0.5, help='Maximum initial delay between requests in seconds (default: 0.5)')
    parser.add_argument('--backoff-max', type=float, default=10.0, help='Maximum delay for backoff in seconds (default: 10.0)')
    parser.add_argument('--retries', '-r', type=int, default=5, help='Maximum number of retries for failed requests (default: 5)')
    parser.add_argument('--interactive', '-i', action='store_true', help='Run in interactive mode')
    parser.add_argument('--clear-history', action='store_true', help='Clear the history of checked domains')
    ...
    if args.clear_history:
        if os.path.exists(CHECKED_DOMAINS_FILE):
            ...
            print(f"Cleared checked domains history")
    ...
    if args.interactive:
        ...
        while True:
            ...
            try:
                num_names = int(input(f"How many names to generate? [{args.num}]: ") or str(args.num))
            except ValueError:
                num_names = args.num
